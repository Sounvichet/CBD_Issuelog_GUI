...
public partial class Payment_biller_disp_index : System.Web.UI.Page
{
    ...
    {
        _grid._GridviewBinding(grid1, _biller._get_biller_pending());
        Label4.Text = grid1.Rows.Count.ToString();
    }
    protected void btnNew_Click(object sender, EventArgs e)
    {
        Response.Redirect("~/Payment/new_biller_dispute.aspx");
    }
    protected void btnedit_click(object sender, EventArgs e)
    {
        foreach (GridViewRow row in grid1.Rows)
        {
            ...
            if (checkbox.Checked)
            {
                ...
                Response.Redirect("~/Payment/edit_biller_dispute.aspx?original_ref=" + id);
            }
        }
    }
    protected void btndel_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow row in grid1.Rows)
        {
            ...
            {
                string id = Convert.ToString(grid1.DataKeys[row.RowIndex].Value);
                Response.Redirect("~/Payment/delete_biller_dispute.aspx?original_ref=" + id);
            }
        }
    }
    ...
    private void _getexcelticketlisting()
    {
        ...
        dt = _biller._get_biller_pending();
