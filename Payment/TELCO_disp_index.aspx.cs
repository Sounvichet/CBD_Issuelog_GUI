...
public partial class Payment_TELCO_disp_index : System.Web.UI.Page
{
    ...
    {
        _grid._GridviewBinding(grid1, _telco._get_telco_disputed_list());
        Label4.Text = grid1.Rows.Count.ToString();
    }
    protected void btnNew_Click(object sender, EventArgs e)
    {
        Response.Redirect("~/Payment/new_telco_dispute.aspx");
    }
    protected void btnedit_click(object sender, EventArgs e)
    {
        foreach (GridViewRow row in grid1.Rows)
        {
            ...
            if (checkbox.Checked)
            {
                ...
                Response.Redirect("~/Payment/edit_telco_dispute.aspx?HTB_EX_REF=" + id);
            }
        }
    }
    protected void btndel_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow row in grid1.Rows)
        {
            ...
            {
                string id = Convert.ToString(grid1.DataKeys[row.RowIndex].Value);
                Response.Redirect("~/Payment/delete_telco_dispute.aspx?HTB_EX_REF=" + id);
            }
        }
    }
    protected void grid1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "lnkedit")
        {
            ...
            Response.Redirect("~/Payment/edit_telco_dispute.aspx?HTB_EX_REF=" + row.Cells[1].Text);
        }
        else if (e.CommandName == "InkDelete")
        {
            int index = Convert.ToInt32(e.CommandArgument);
            GridViewRow row = grid1.Rows[index];
            Response.Redirect("~/ticket/delete_telco_dispute.aspx?HTB_EX_REF=" + row.Cells[1].Text);
        }
    }
    ...
    private void _getexcelticketlisting()
    {
        ...
        dt = _telco._get_telco_disputed_list();
