...
public partial class SERVICES_edit : System.Web.UI.Page
{
    ...
    private bool _SERVICE_UPDATE()
    {
       // string getdate = DateTime.Now.ToString("dd/MMM/yyyy HH:mm:ss").Replace('/', '-');
        ...
        {
            _service._price = Convert.ToDecimal(txtprice.Text);
        }
        _service._serviceNameK = txtservicename.Text;
        _service._Status = D_ServiceStatus.SelectedValue;
        _service._USERiD = Session["USER_NAME"].ToString();
        _service._RemoteAddr = _getlocalIP();
        bool _getService = _service._SERVICE_UPDATE();
        ...
        {
            ShowMessage("SERVICE Registration Successfully : "+ _service._ServiceID + "", MessageType.Success);
        }
        return _getService;
    }
    ...
    protected void linkbtnupdate_Click(object sender, EventArgs e)
    {
        if (txtprice.Text == "")
        {
            ShowMessage("Maindatory field is price not allow with null", MessageType.Error);
        }
        else
        {
            _SERVICE_UPDATE();
